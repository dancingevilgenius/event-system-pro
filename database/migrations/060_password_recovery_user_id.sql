-- Allow password recovery setup RPCs to use p_user_id (same pattern as change_password).
-- Run: psql -U postgres -d event_system_pro -f database/migrations/060_password_recovery_user_id.sql

\connect event_system_pro

DROP FUNCTION IF EXISTS api.get_password_recovery_setup();
DROP FUNCTION IF EXISTS api.update_password_recovery(json);

CREATE OR REPLACE FUNCTION api.get_password_recovery_setup(p_user_id bigint DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  v_ids json;
BEGIN
  v_target_user_id := api.current_user_id();

  IF p_user_id IS NOT NULL THEN
    IF v_target_user_id IS NOT NULL AND v_target_user_id <> p_user_id THEN
      RETURN json_build_object(
        'ok', false,
        'message', 'You can only view your own password recovery settings.'
      );
    END IF;
...
CREATE OR REPLACE FUNCTION api.update_password_recovery(
  p_answers json,
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    IF v_user_id IS NOT NULL AND v_user_id <> p_user_id THEN
      RETURN json_build_object(
        'ok', false,
        'message', 'You can only update your own password recovery settings.'
      );
    END IF;
    v_user_id := p_user_id;
  END IF;
...
GRANT EXECUTE ON FUNCTION api.get_password_recovery_setup(bigint) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION api.update_password_recovery(json, bigint) TO anon, authenticated;
