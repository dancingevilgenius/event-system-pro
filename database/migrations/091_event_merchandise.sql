-- event_merchandise: per-event merchandise configuration (1:1 with event via event_code).
...
CREATE TABLE IF NOT EXISTS public.event_merchandise (
  ...
  CONSTRAINT event_merchandise_fk_event FOREIGN KEY (event_code)
    REFERENCES public."event" (event_code) ON DELETE RESTRICT ON UPDATE RESTRICT
);

DROP TRIGGER IF EXISTS trg_set_modified_audit ON public.event_merchandise;
...
DROP TRIGGER IF EXISTS trg_audit_row_change ON public.event_merchandise;
CREATE TRIGGER trg_audit_row_change
  AFTER INSERT OR UPDATE OR DELETE ON public.event_merchandise
  FOR EACH ROW
